 ...
 * 
 * You are given a 2D integer array drones, where drones[i] = [xi, yi, rangei] represents the x-coordinate, y-coordinate, and travel range of the i^th drone.
 * 
...
public:
    int nearestDrone(vector<vector<int>>& drones, vector<int>& target) {
        int ans = -1, minDist = INT_MAX, i=0;
        for(auto it: drones){
            int dist = abs(it[0]-target[0])+abs(it[1]-target[1]);
            if(dist<=it[2] and dist<minDist){
                minDist = dist;
                ans=i;
            }
            i++;
