...
int main() {
    ...
    while(t--){
        ...
        for(int i=1; i<n; i++){
           gap= max({ref[i]-ref[i-1], gap});
        }
        int lastGap = 2*(x-ref[n-1]);
        cout<<max(gap, lastGap)<<'\n';
    }
